...
@endphp
@if(isset($editLesson))
    <form class="form-horizontal" method="POST" action="{{ route('update-course-quiz') }}" enctype="multipart/form-data" id="question_bank">
        @csrf
        @else
            @if (permissionCheck('question-bank.store'))
                <form class="form-horizontal" method="POST" action="{{ route('save-course-quiz') }}" enctype="multipart/form-data" id="question_bank">
                    @csrf
                    @endif
                    @endif
