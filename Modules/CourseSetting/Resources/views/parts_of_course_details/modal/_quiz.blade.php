<div class="modal-dialog modal-lg modal-dialog-centered student-details">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <form
                @if($edit)
                    class="form-horizontal" method="POST" action="{{ route('update-course-quiz') }}"
                enctype="multipart/form-data" id="question_bank">
                @csrf
                @else
                    @if (permissionCheck('question-bank.store'))
                        class="form-horizontal" method="POST" action="{{ route('save-course-quiz') }}"
                        enctype="multipart/form-data" id="question_bank">
                        @csrf
                    @endif
                @endif
                ...
                <input type="hidden" name="lesson_id" value="{{@$edit->id}}">
                @if (isset($course->subcategory_id))
                    <input type="hidden" name="sub_category" value="{{@$course->subcategory_id}}">
                @endif
                <input type="hidden" name="chapterId" value="{{@$chapter_id}}">
                <input type="hidden" name="quiz_id" value="{{@$edit->lessonQuiz->id}}">
                ...
                <strong>{{ $errors->first('quiz') }}</strong>
                            ...
                            <div class="input-effect mt-2 pt-1" id="existing_quiz">
                                <label class="primary_input_label mt-1" for=""> {{__('quiz.Quiz')}} <span
                            ...
                            {{-- Start New Create --}}
                            <div class="new_quiz mt-20" style="display: {{!$edit?'none':''}}">
                                                ...
                                                <div class="col-lg-12">
                                                    <div class="input-effect mt-3">
                                                        ...
                                                        <input {{ $errors->has('title') ? ' autofocus' : '' }}
                                                               ...
                                                               autocomplete="off"
                                                               value="{{isset($edit->lessonQuiz)? $edit->lessonQuiz->getTranslation('title',$language->code): ''}}">
                                                        <input type="hidden" name="id"
                                                               value="{{isset($edit->lessonQuiz)? $edit->lessonQuiz->id: ''}}">
                                                ...
                                                <div class="col-lg-12">
                                                    <div class="input-effect">
                                                        ...
                                                        <textarea {{ $errors->has('instruction') ? ' autofocus' : '' }}
                                                                  ...
                                                                  cols="0" rows="4"
                                                                  name="instruction[{{$language->code}}]">{{isset($edit->lessonQuiz)? $edit->lessonQuiz->getTranslation('instruction',$language->code): ''}}</textarea>
                                ...
                                <div class="row mt-25">
                                    <div class="col-lg-12">
                                        <div class="input-effect">
                                            <label class="primary_input_label mt-1"
                                                   for="">{{__('quiz.Minimum Percentage')}}
                                                <span class="required_mark">*</span> </label>
                                            <input {{ $errors->has('title') ? ' percentage' : '' }}
                                                   ...
                                                   value="{{isset($edit->lessonQuiz)? $edit->lessonQuiz->percentage: old('percentage')}}">
                                            <input type="hidden" name="id"
