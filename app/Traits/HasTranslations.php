...
trait HasTranslations
{
    ...
    public function getTranslations(string $key = null): array
    {
        if ($key !== null) {
            ...
            $row_value =$this->getAttributes()[$key]??'';
            $json_value =json_decode($row_value ?? '' ?: '{}', true);

            if (!empty($row_value) && empty($json_value)){
                $code = app()->getLocale() ?? 'en';
                $json_value[$code]= $row_value;
            }


            return array_filter((array)$json_value ?: [], function ($value){
