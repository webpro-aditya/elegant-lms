...
if (!function_exists('SaasDomain')) {
    function SaasDomain()
    {
        ...
        if (file_exists($saas_module) || file_exists($saas_module2)) {
            ...
            $module_status = json_decode(file_get_contents($modulesPath), true);
...
if (!function_exists('getFullDomain')) {
    function getFullDomain($domain)
    {
        ...
        $protocol = parse_url($mainDomain, PHP_URL_SCHEME) . '://';
       return $protocol . $domain . '.' . config('app.short_url');    }
}
...
if (!function_exists('tenant_route')) {
    function tenant_route($route)
    {
        ...
        if(isModuleActive('LmsSaas')){
            $oldDomain = route($route);
            $lms_info = SaasInstitute();
            $protocol = parse_url($oldDomain, PHP_URL_SCHEME) . '://';

            $domain = $lms_info && isset($lms_info->domain) ? $lms_info->domain : '';
            return $protocol . $domain . '.' . config('app.short_url') . parse_url($oldDomain, PHP_URL_PATH);
        }else{
            return route($route);
        }
    }
